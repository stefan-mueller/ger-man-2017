...
dfm_man <- dfm(corpus_ger_man_2017, remove = c(stopwords("german"), stopwords_additional),
               tolower = TRUE, remove_punct = TRUE, remove_numbers = TRUE, remove_url = TRUE)
...
ggplot(data = freq_ordered, aes(x = order, y = frequency)) +
  ...
  labs(x = NULL, y = "Relative word frequency") + 
  theme_custom()
...
## Run Wordfish model
model_wordfish <- textmodel_wordfish(dfm_man)
